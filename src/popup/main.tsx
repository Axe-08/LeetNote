...
import { StorageKey, FullSettingsObject } from '../shared/types';
...
const SettingsPanel = ({ onClose }: { onClose: () => void }) => {
  ...
  const loadSettingsData = async () => {
    ...
    const qResp = await sendMessage<{ entries: any[] }>('GET_QUEUE_STATUS');
    if (qResp.success && qResp.data) {
              ...
              queue.map((entry) => (
                <div key={entry.id} className="q-item">
                  <div className="q-details">
                    <div className="q-title">{entry.payload.metadata.title}</div>
                    <div className="q-desc">Attempt #{entry.attemptCount || 1} · Offline</div>
                  </div>
